...
import (
	"context"
	"github.com/uptsmart/uptick-sdk-go/store/gaskv"
	"github.com/uptsmart/uptick-sdk-go/store/types"
	"time"

	"github.com/gogo/protobuf/proto"
	stypes "github.com/uptsmart/uptick-sdk-go/store"
...
func NewContext(ms types.MultiStore, header tmproto.Header, isCheckTx bool, logger log.Logger) Context {
...
func (c Context) WithMultiStore(ms types.MultiStore) Context {
	c.ms = ms
	return c
}
...
func (c Context) KVStore(key types.StoreKey) types.KVStore {
	return gaskv.NewStore(c.MultiStore().GetKVStore(key), c.GasMeter(), stypes.KVGasConfig())
}

// TransientStore fetches a TransientStore from the MultiStore.
func (c Context) TransientStore(key types.StoreKey) types.KVStore {
