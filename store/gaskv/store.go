...
func (gs *Store) Get(key []byte) (value []byte) {
	gs.gasMeter.ConsumeGas(gs.gasConfig.ReadCostFlat, store.GasReadCostFlatDesc)
	value = gs.parent.Get(key)

	// TODO overflow-safe math?
	gs.gasMeter.ConsumeGas(gs.gasConfig.ReadCostPerByte*types.Gas(len(value)), store.GasReadPerByteDesc)
...
func (gs *Store) Set(key []byte, value []byte) {
	...
	gs.gasMeter.ConsumeGas(gs.gasConfig.WriteCostFlat, store.GasWriteCostFlatDesc)
	// TODO overflow-safe math?
	gs.gasMeter.ConsumeGas(gs.gasConfig.WriteCostPerByte*types.Gas(len(value)), store.GasWritePerByteDesc)
...
func (gs *Store) Has(key []byte) bool {
	...
	gs.gasMeter.ConsumeGas(gs.gasConfig.HasCost, store.GasHasDesc)
...
func (gs *Store) Delete(key []byte) {
	...
	gs.gasMeter.ConsumeGas(gs.gasConfig.DeleteCost, store.GasDeleteDesc)
...
func (gs *Store) CacheWrapWithListeners(_ types.StoreKey, _ []store.WriteListener) types.CacheWrap {
...
func (gi *gasIterator) consumeSeekGas() {
	...
	gi.gasMeter.ConsumeGas(gi.gasConfig.ReadCostPerByte*types.Gas(len(value)), store.GasValuePerByteDesc)
	gi.gasMeter.ConsumeGas(gi.gasConfig.IterNextCostFlat, store.GasIterNextCostFlatDesc)
}
